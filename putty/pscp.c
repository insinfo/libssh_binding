...
static void (*callback_stats_func)(char const* output_stats);
                        ...
                        time_t start, time_t now)
{
    ...
    if (callback_stats_func == NULL) {
        ...
        //isaque adicionei a callback_stats_func
        char* estatisticas = dupprintf("\r%-25.25s | %"PRIu64" kB | %5.1f kB/s | "
            "ETA: %8s | %3d%%", name, done >> 10,
            ratebs / 1024.0, etastr, pct);
        callback_stats_func(estatisticas);
        free(estatisticas);
    }
 ...
 */
int psftp_main(int argc, char *argv[], void (*callback_stats)(char const* output_stats))
{
    callback_stats_func = callback_stats;
